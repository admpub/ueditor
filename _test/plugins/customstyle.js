...
test( 'block的元素-样式的反复转换：块-块；包含的选取方式：整段、部分、多段', function () {
    ...
    hStyle = $(editor.body.getElementsByTagName('h1')[0]);
    ok(  editor.body.getElementsByTagName('h1')[0].style.borderBottomWidth == '2px' , '检查边框宽' );
    ok(  hStyle.css( 'border-bottom-style' ) == 'solid' , '检查边框风格' );
    ok(  hStyle.css( 'padding-bottom' ) == '0px'&&hStyle.css( 'padding-top' ) == '0px'&&hStyle.css( 'padding-left' ) == '0px'&&hStyle.css( 'padding-right' ) == '4px' , '检查padding' );
    ...
    hStyle = $( editor.body.getElementsByTagName('h3')[0]);
    ...
    hStyle = $( editor.body.getElementsByTagName('h3')[1] );
...
test( 'block的元素-样式的反复转换：块-块；包含的选取方式： 多段部分', function () {
    ...
    hStyle = $( editor.body.getElementsByTagName('h1')[1] );
    ...
    hStyle = $( editor.body.getElementsByTagName('h1')[2] );
...
test( 'block的元素-样式的反复转换：块-内联；包含的选取方式：闭合、多段部分', function () {
    ...
    hStyle = $( editor.body.getElementsByTagName('span')[1] );
    if ( ua.browser.webkit )
        ok(  hStyle.css( 'font-style' ) == 'italic'&&hStyle.css( 'font-weight' ) == 'bold' , '检查字体' );
    else
         ok(  hStyle.css( 'font-style' ) == 'italic'&&hStyle.css( 'font-weight' ) == '700' , '检查字体' );
...
test( 'block的元素-样式的反复转换：内联-块；包含的选取方式：闭合', function () { //从内联-块的转换意义不大，其实还是针对块的转换，就不做多种方式选取了
    ...
    hStyle = $( editor.body.getElementsByTagName('span')[0] );
    if ( ua.browser.webkit )
        ok(  hStyle.css( 'font-style' ) == 'italic'&&hStyle.css( 'font-weight' ) == 'bold' , '检查字体' );
    else
        ok(  hStyle.css( 'font-style' ) == 'italic'&&hStyle.css( 'font-weight' ) == '700' , '检查字体' );
...
//TODO bug
test( 'trace 3388：设置明显强调后再设置强调', function() {
    var editor = te.obj[0];
    var range = te.obj[1];
    editor.setContent( '<p>hello</p>' );
    range.setStart(editor.body.firstChild.firstChild,0).setEnd(editor.body.firstChild.firstChild,4).select();
    editor.execCommand('customstyle', {
        tag:'span',
        label:'明显强调',
        style:'font-style:italic;font-weight:bold;color:rgb(51, 153, 204)'
    });
    ua.checkHTMLSameStyle('<p><span label="明显强调" style="font-style: italic; font-weight: bold; color: rgb(51, 153, 204);">hell</span>o</p>',editor.document,editor.body,'明显强调');
//    range.setStart(editor.body.firstChild.firstChild.firstChild,0).setEnd(editor.body.firstChild.firstChild.firstChild,4).select();
//    editor.execCommand('customstyle', {
//        tag:'span',
//        label:'强调',
//        style:'font-style:italic;font-weight:bold;color:#000'
//    });
//    equal(editor.body.innerHTML, '<p><span label="强调" style="font-style: italic; font-weight: bold;">hell</span>o</p>', '强调' );
});